...
def collate_im_files(folder: str) -> list:
    ...
    im_files = [
        ...
        os.path.join(folder, file) for file in folder_files 
...
def save_results(raw_frcs: 'np.ndarray', denoised_frcs: 'np.ndarray',
...
def main():
    ...
    for i, file in enumerate(zip(noisy_im_files, denoised_im_files)):
        ...
        denoised_rfrcs[i, 0], denoised_rfrcs[i, 1], denoised_rfrcs[i, 2] = process_image(file[1])

    all_data = save_results(noisy_rfrcs, denoised_rfrcs,
                            noisy_im_files, denoised_im_files, opt.output_folder)
